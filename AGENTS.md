...
- **Guide contract:** `device.json` boots a catalog screen from
  `browse.url`. Paging is URL-driven: keep ≤ `page_size` items per page file
  (`guide-1.json`, then `guide-2.json`, …). Firmware caps: `device.json` <
  8 KB, `page_size` ≤ 16, browse response ≤ 1 MB.
